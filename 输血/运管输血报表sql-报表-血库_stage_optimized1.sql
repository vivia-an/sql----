...
blood_inventory_consolidated AS (
    SELECT 
        ...
        COALESCE((
            ...
            WHERE inv.inventory_time >= tr.start_date 
                AND inv.inventory_time <= tr.end_date
                AND inv.isdeleted = '0'
        ), 0) as "盘库" 
    FROM time_ranges tr
    ...
    INNER JOIN hid0101_orcl_lis_xhdata.LIS6_INSPECT_SAMPLE d
        ...
        AND d."isdeleted" = '0'
    WHERE a."isdeleted" = '0'
    GROUP BY tr.period_id, tr.period_label, tr.period_name, tr.start_date, tr.end_date, tr.start_date, tr.end_date
),
    ...
    END as "周末加班手术台次",
       CASE WHEN MAX(CASE WHEN "排序" = 3 THEN "盘库" END) > 0 
         THEN ROUND((MAX(CASE WHEN "排序" = 1 THEN "盘库" END) - 
                    ...
                    MAX(CASE WHEN "排序" = 3 THEN "盘库" END), 2)
         ELSE NULL 
    END as "盘库",
