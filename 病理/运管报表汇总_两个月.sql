...
last_last_month_range AS (
  SELECT
    FORMAT_DATETIME(DATE_TRUNC('MONTH', CURRENT_DATE - INTERVAL '3' MONTH), 'yyyy-MM-dd') || ' 00:00:00' AS start_date,
    FORMAT_DATETIME(LAST_DAY_OF_MONTH(DATE_TRUNC('MONTH', CURRENT_DATE - INTERVAL '3' MONTH)), 'yyyy-MM-dd') || ' 23:59:59' AS end_date
),

last_year_same_month_range AS (
  SELECT
    FORMAT_DATETIME(DATE_TRUNC('MONTH', CURRENT_DATE - INTERVAL '14' MONTH), 'yyyy-MM-dd') || ' 00:00:00' AS start_date,
    FORMAT_DATETIME(LAST_DAY_OF_MONTH(DATE_TRUNC('MONTH', CURRENT_DATE - INTERVAL '14' MONTH)), 'yyyy-MM-dd') || ' 23:59:59' AS end_date
),
...
last_month_income AS (
  ...
  WHERE IsDeleted = '0' 
    AND RecDeptName IN ('病理科','锦江病理科','温江病理科','天府病理科')
    AND SUBSTRING(chargedttm, 1, 7) = 
        FORMAT_DATETIME(DATE_TRUNC('MONTH', CURRENT_DATE - INTERVAL '3' MONTH), 'yyyy-MM')
),
...
last_year_income AS (
  ...
  WHERE IsDeleted = '0' 
    AND RecDeptName IN ('病理科','锦江病理科','温江病理科','天府病理科')
    AND SUBSTRING(chargedttm, 1, 7) = 
        FORMAT_DATETIME(DATE_TRUNC('MONTH', CURRENT_DATE - INTERVAL '14' MONTH), 'yyyy-MM')
),
...
last_month_physical AS (
  SELECT SUM("计算金额") AS income_value FROM (
    ...
    WHERE ((f_feecharged = 'AQ==') OR f_feecharged = 'true')
      ...
      AND SUBSTRING(dateregister, 1, 7) = DATE_FORMAT(DATE_ADD('month', -3, CURRENT_DATE), '%Y-%m')
      AND medorgcode IN ('HID0101')
  ) t2
),
...
last_year_physical AS (
  SELECT SUM("计算金额") AS income_value FROM (
    ...
    WHERE ((f_feecharged = 'AQ==') OR f_feecharged = 'true')
      ...
      AND SUBSTRING(dateregister, 1, 7) = DATE_FORMAT(DATE_ADD('month', -14, CURRENT_DATE), '%Y-%m')
      AND medorgcode IN ('HID0101')
  ) t3
),
...
final_results AS (
  -- 1. 检查治疗人次/项次
  SELECT 
    FORMAT_DATETIME(DATE_TRUNC('MONTH', CURRENT_DATE - INTERVAL '2' MONTH), 'yyyy-MM') AS "统计月",
    '主院区' AS "单位",
    '检查治疗人次/项次' AS "项目",
