...
-- 2. m1.mdr_income - 收入表（各院区检查治疗收入、穿刺中心收入）
...
main_income AS (
  ...
  FROM m1.mdr_income, month_range
...
-- 主院区结果
main_results AS (
  SELECT (SELECT month_label FROM month_range) AS "统计月", '主院区' AS "单位", '检查治疗人次/项次' AS "项目", COALESCE((SELECT count_value FROM main_count), 0) AS "数值"
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '主院区', '检查治疗收入(元)', COALESCE((SELECT income_value FROM main_income), 0)
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '主院区', '穿刺中心细胞病理收入(元)', COALESCE((SELECT income_value FROM main_puncture), 0)
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '主院区', '本部温江第三方体检病理收入(元)', COALESCE((SELECT income_value FROM main_physical), 0)
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '主院区', '其他收入(元)', 0
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '主院区', '本部收入合计', 
    ...
    COALESCE((SELECT income_value FROM main_physical), 0)
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '主院区', '领用材料试剂费(元)', COALESCE((SELECT cost_value FROM main_material), 0)
  UNION ALL
...
-- 上锦院区结果
sj_results AS (
  SELECT (SELECT month_label FROM month_range) AS "统计月", '上锦院区' AS "单位", '检查治疗人次/项次' AS "项目", COALESCE((SELECT count_value FROM sj_count), 0) AS "数值"
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '上锦院区', '上锦检查治疗收入(元)', COALESCE((SELECT income_value FROM sj_income), 0)
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '上锦院区', '上锦体检病理收入(元)', COALESCE((SELECT income_value FROM sj_physical), 0)
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '上锦院区', '上锦收入合计', 
    COALESCE((SELECT income_value FROM sj_income), 0) + 
    COALESCE((SELECT income_value FROM sj_physical), 0)
),

-- 天府院区结果
tf_results AS (
  SELECT (SELECT month_label FROM month_range) AS "统计月", '天府院区' AS "单位", '检查治疗人次/项次' AS "项目", COALESCE((SELECT count_value FROM tf_count), 0) AS "数值"
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '天府院区', '天府检查治疗收入(元)', COALESCE((SELECT income_value FROM tf_income), 0)
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '天府院区', '天府体检病理收入(元)', COALESCE((SELECT income_value FROM tf_physical), 0)
  UNION ALL
  SELECT (SELECT month_label FROM month_range), '天府院区', '天府收入合计', 
    COALESCE((SELECT income_value FROM tf_income), 0) + 
    COALESCE((SELECT income_value FROM tf_physical), 0)
),
...
total_count AS (
  SELECT 
    ...
    '合计' AS "单位",
    '检查治疗人次/项次合计' AS "项目",
    SUM(CASE WHEN "项目" = '检查治疗人次/项次' THEN "数值" ELSE 0 END) AS "数值"
  FROM all_results
),
