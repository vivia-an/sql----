...
wb_bufe_last_month AS (
    ...
    WHERE STRPOS(B."his_id", '||') > 0 
        AND A."patient_type" IN ('1','2','3','4','5','8','12')
        AND A."GROUP_ID" IN ('G999','G998')
        AND A."inspection_date" BETWEEN d.last_month_start AND d.last_month_end
        AND A."isdeleted" = '0'
),

-- 按科室统计（上月）
keshi_stat_last_month AS (
    SELECT 
        COUNT(DISTINCT A."requisition_id") as "样本数"
    FROM hid0101_orcl_lis_dbo.lis_inspection_sample A
    INNER JOIN hid0101_orcl_lis_dbo.lis_requisition_item B 
...
wb_bufe_last_year AS (
    ...
    WHERE STRPOS(B."his_id", '||') > 0 
        AND A."patient_type" IN ('1','2','3','4','5','8','12')
        AND A."GROUP_ID" IN ('G999','G998')
        AND A."inspection_date" BETWEEN d.last_year_start AND d.last_year_end
        AND A."isdeleted" = '0'
),

-- 按科室统计（去年同期）
keshi_stat_last_year AS (
    SELECT 
        COUNT(DISTINCT A."requisition_id") as "样本数"
    FROM hid0101_orcl_lis_dbo.lis_inspection_sample A
    INNER JOIN hid0101_orcl_lis_dbo.lis_requisition_item B 
