...
%
%  Same manuscript as main_applsci.tex, compiled with MDPI's own class rather
%  than an emulation of it. The class and journalnames.tex come from MDPI's
%  distribution. Two local substitutions are needed on this machine and must be
%  removed before submission:
%    * enotez.sty is stubbed (mdpi.cls loads it for endnotes, which this paper
%      does not use; the real package needs a LaTeX kernel newer than TL2013)
%    * the four MDPI logo files are blank placeholders; the real artwork ships
%      with MDPI's official template download
%==============================================================================
\documentclass[applsci,article,submit,pdftex,moreauthors]{mdpi}
...
%--- LaTeX stops placing floats ("Too many unprocessed floats"). morefloats is
%--- not on this machine and this kernel predates \extrafloats, so the list is
%--- extended here by the same mechanism those provide.
\makeatletter
\newcount\mf@i
...
%--- local only: this machine has no ghostscript, so the .eps logo names that
%--- mdpi.cls hard-codes are served by PDF files carrying that extension.
...
%--- mdpi.cls ships captionsetup for figure/specialtable but not for the
%--- plain table environment; this gives table captions the same MDPI style.
\captionsetup[table]{position=top, labelfont={bf, small, stretch=1.17},
  labelsep=period, textfont={small, stretch=1.17}, aboveskip=6pt,
...
\Author{\PH{Author One} $^{1}$,
\PH{Author Two} $^{2}$,
\PH{Author Three} $^{2}$,
\PH{Author Four} $^{3}$,
\PH{Author Five} $^{4}$,
\PH{Author Six} $^{3}$,
\PH{Author Seven} $^{5}$ and
\PH{Author Eight} $^{1,5,}$*}

\AuthorNames{\PH{Author One}, \PH{Author Two}, \PH{Author Three},
\PH{Author Four}, \PH{Author Five}, \PH{Author Six}, \PH{Author Seven},
\PH{Author Eight}}

\address{%
$^{1}$ \quad Department of Medical Informatics, \PH{Hospital Name}, \PH{University}, \PH{City} \PH{Postcode}, China\\
$^{2}$ \quad Big Data Platform Center, \PH{Hospital Name}, \PH{City} \PH{Postcode}, China\\
$^{3}$ \quad Department of Anesthesiology and Surgical Management, \PH{Hospital Name}, \PH{City} \PH{Postcode}, China\\
$^{4}$ \quad Department of Laboratory Medicine and Blood Transfusion, \PH{Hospital Name}, \PH{City} \PH{Postcode}, China\\
$^{5}$ \quad Department of Quality Management, \PH{Hospital Name}, \PH{University}, \PH{City} \PH{Postcode}, China}
...
\begin{tikzpicture}[
  ...
  io/.style   ={draw,rounded corners=2pt,fill=gray!12,align=center,
                text width=18mm,minimum height=8mm,inner sep=2pt},
  st/.style   ={draw,fill=blue!7,align=center,text width=21mm,
                minimum height=11mm,inner sep=2.5pt},
  mod/.style  ={draw,fill=orange!10,align=center,text width=24mm,
                minimum height=12mm,inner sep=2.5pt},
  kb/.style   ={draw,fill=green!8,align=center,text width=22mm,
...
\authorcontributions{Conceptualization, \PH{A.A.}, \PH{B.B.} and \PH{C.C.};
methodology, \PH{B.B.} and \PH{D.D.}; software, \PH{B.B.} and \PH{E.E.};
validation, \PH{E.E.}, \PH{F.F.} and \PH{G.G.}; formal analysis, \PH{D.D.} and
\PH{E.E.}; investigation, \PH{B.B.} and \PH{D.D.}; resources, \PH{C.C.} and
\PH{H.H.}; data curation, \PH{D.D.}; writing---original draft preparation,
\PH{B.B.}; writing---review and editing, \PH{A.A.}, \PH{C.C.} and \PH{G.G.};
...
\PH{Hospital Name} (protocol code \PH{XXXX-XXXX}, approved
\PH{DD Month YYYY}).}
...
detector, together with the SynHDW generator and its schema manifest, is openly
available at \PH{repository URL}. The de-identified metadata schema, the
metric-criterion dictionary and the evaluation task set are available from the
corresponding author on reasonable request, subject to the data-governance rules
of \PH{Hospital Name}. Patient-level warehouse data cannot be shared.}

\acknowledgments{The authors thank \PH{names} for reviewing the criterion
definitions used as reference standards, and the data engineering team of
\PH{Hospital Name} for access to the reporting ticket archive.}
...
%--- mdpi.cls opens paracol{2} at \maketitle (narrow citation column + wide
%--- body column) and leaves it open; the document closes it.
\end{paracol}
