...
%  ！！已退役 —— 不可投稿 ！！
%
%  这份文件是 JMIR Med Inform 体例的\emph{复刻练习稿}：它把
%  Lee KH 等 (JMIR Med Inform 2025;13:e71252) 的版面与结构 1:1 复制了下来，
%  署名是 10 处 \PH{Author N} 占位作者。**它不是投稿候选，任何情况下都不要提交。**
%
%  取代它的是 main_applsci_mdpicls.tex（Applied Sciences，官方 mdpi.cls，
%  真实单作者）。本文件保留的理由只有一个：它的 JMIR 版面家具是对着真刊 PDF
%  量出来的，main_jmir.tex 复用了那套测量；verify_structure.py 也仍拿它做
%  第二个 profile。除此之外不要动它，也不要把它的内容当作我们的主张。
%
%  沿用 e71252 评测框架一事，已在 main_applsci_mdpicls.tex 中按学术规范
%  逐处标注并引用（见 ../LOOP_STATE.md 轮 5）。
%==============================================================================
%==============================================================================
...
\usepackage[margin=0.9in,top=0.92in,bottom=0.92in]{geometry}
...
\usepackage{balance}      % balance the columns on the last two-column page
...
\raggedbottom
% full-width floats are top-only in twocolumn; give them room
...
% source paper), so it cannot live inside \twocolumn[...]; switch columns later.
\onecolumn
...
\noindent\textbf{Objective:} The aim of this study is to develop an automated system converting free-text hospital statistical requests into Presto Structured Query Language (SQL) queries compatible with a three-layer clinical data warehouse (CDW) and to
...
pipeline (segmentation, filtering, and simplification) achieving 56.3\% token
...
deployed in our metadata portal, using 398 business terms from 30 reporting
...
\noindent\textbf{Results:} The LLM achieved a 52.3\% mapping accuracy versus
RBFM's 32.9\% ($P<.001$), with domain-specific performance ranging from 70.7\%
(surgery and anesthesia) to 34.5\% (laboratory). Surprisingly, the on-premises
qwen2.5:7b model achieved the highest effective SQL rate (71.7\%) compared to
gpt-4o (60.8\%), attributed to lower hallucination rates (20.4\% vs 37.6\%).
The overall hallucination rate was 34.6\%, with wrong criterion assignments
(34.7\%) and placeholder insertions (28.5\%) being the most common. Result-set
validation revealed mixed performance: high concordance for elective surgery
volume (Jaccard=0.79), complete failure for unplanned reoperation
(Jaccard=0.00), and minimal overlap for emergency surgery volume (Jaccard=0.08),
despite perfect overlap coefficients in both surgery-volume cases. Moderate
performance was observed for critical laboratory value exposure (Jaccard=0.27).
...
Mapping accuracy was assessed on the 30-task development set (surgery and anesthesia, blood transfusion, laboratory), which spans a wide range of institutional terminology. We compared the LLM with the rule-based field matcher (RBFM) currently deployed in our metadata portal. While the LLM
...
Notably, among the 267 terms that RBFM misclassified, the LLM correctly mapped 72 (27.0\%), demonstrating superior contextual understanding. For instance, RBFM
...
Output is also sensitive to prompt design and context length. Requests carrying vague or clinically interpreted terms (``major surgery'', ``recent infection'') are the hardest: the system overgeneralizes them or drops a qualifier such as the time window, and these appear in our error analysis mainly as wrong criterion assignments and placeholder insertions. A governed indicator catalog or a rule-based disambiguation step would likely be needed to fix this. Four reporting gaps are worth naming, and the completed MI-CLAIM and MI-CLAIM-GEN checklists in Multimedia Appendix 4 record each against the item it fails: no comparison on a public text-to-SQL benchmark, so absolute standing is unestablished; expert raters not blinded to the generating model; one execution per model--prompt--task cell, leaving run-to-run variability unestimated; and, most consequential for the argument here, an abstention path that the paper relies on throughout but never measures. We do not report how often the system abstained, how often it was right to, or how often it committed to one reading of an ambiguous criterion when it should have asked. Quantifying abstention quality is the most useful single addition to this work.
...
\section*{Multimedia Appendix 4}
Completed MI-CLAIM and MI-CLAIM-GEN checklists.\\
{\footnotesize [DOCX File (Microsoft Word File), \PH{XX} KB-Multimedia Appendix 4]}
