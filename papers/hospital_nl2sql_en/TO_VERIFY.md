...
投稿文件是 **`main_applsci_mdpicls.tex`**（仓库内官方 `Definitions/mdpi.cls` 2025-06-18 版，
Applied Sciences 版式）；`main_applsci.tex` 是同内容的 article 类备份，两者
正文、图、表、参考文献逐字一致。

作者与单位已于 2026-07-27 落实（李世凯，四川大学华西医院信息与数据管理部，
单一作者）。`\documentclass` 随之改为 `oneauthor`，该选项会抑制 `mdpi.cls`
的带星通讯行，因此联系方式按 MDPI 单作者版式写在单位行内，作者名不再带
角标与星号。

`grep -o 'PH{' main_applsci_mdpicls.tex | wc -l` → 当前 **9 处**：

| 行 | 内容 | 谁来填 |
|---|---|---|
| 85, 86 | 单位行内的通讯邮箱与电话 | 作者 |
| 138, 139 | Introduction：2023--2024 年工单中等待 >5 个工作日的占比、交付后仍需再澄清口径的占比 | 需从工单库统计 |
| 246, 546, 1489 | IRB 批号，三处必须是同一个号 | 伦理办 |
| 1471 | 补充材料 URL | 投稿系统生成 |
| Funding | 基金号，或"本研究未获外部资助" | 作者 |

IRB 批准日期与 Claude 模型卡访问日期已填为 2026-07-27。

Limitations 里原有一句"预计真实数据会使性能下降 XX%--XX%"已删除并改写：
那个区间没有实验能核，写出来只会被审稿人当成拍脑袋的数字。现在只陈述方向，
并说明界定差距需要一次面向生产表、带裁定真值的前瞻性运行。

期刊卷期年不再是占位符 —— 已按 MDPI 模板自带默认值填入（`\pubvolume{1}`
等），由编辑部在录用后替换，作者无需填。

原 JMIR 式"Edited by / Please cite as / Creative Commons"页脚已整块删除：
那是 JMIR 的版面家具，Applied Sciences 由 `mdpi.cls` 自动排版权块，
留着会被编辑当作套模板的痕迹。删除同时清掉了 16 处占位符。
