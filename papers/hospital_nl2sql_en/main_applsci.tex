...
%--- mdpi.cls supplies \extralength and changepage; define inert stand-ins
%--- so the wide float blocks are byte-identical in both manuscripts.
\usepackage{changepage}
\newlength{\extralength}\setlength{\extralength}{0cm}
\newlength{\fulllength}\setlength{\fulllength}{17.2cm}
...
\vspace{10pt}
\noindent
Shikai Li\,\textsuperscript{1,}*

\vspace{4pt}
{\footnotesize
\noindent\textsuperscript{1}\quad Department of Information and Data Management,
West China Hospital, Sichuan University, Chengdu 610041, China\\[2pt]
...
\small\textbf{Featured Application:} The framework converts free-text statistical requests from
clinical departments into executable, provenance-carrying SQL over a hospital
clinical data warehouse, so that the departmental performance, quality-control
and regulatory indicators on which hospital management depends are delivered with
the counting criterion recorded beside the number rather than left in the query.
It runs on premises, so no patient data leaves the institution, and it refers a
contested criterion back to the indicator owner instead of resolving it
silently.
\end{minipage}}
...
Hospital reporting depends on institution-specific statistical
criteria---which timestamp a case is counted by, which flag defines an
indicator---that carry no cross-site vocabulary, so requests from clinical
departments reach a data engineer before they reach the database. We convert such
requests into executable Presto SQL over a three-layer clinical data warehouse,
grounding generation in a metadata knowledge base and verifying each candidate
through syntax, field, criterion and result checks. On 398 business terms from 30
reporting tasks, mapping accuracy was 52.3\% against 32.9\% for the rule-based
matcher in production ($p<0.001$). A factorial experiment over 24 tasks, 8 models
and 5 prompting strategies produced 960 attempts: 791 yielded SQL, 274 with a
field or criterion error. The
...
not because it generated more. Wrong counting criteria formed the largest error
class at 34.7\%, a failure that yields an executable query and a wrong number,
and that generic schema-linking taxonomies do not capture. Because that failure
silently changes a reported indicator's denominator, we state which
performance-management and quality-control uses the measured reliability
supports, and which risk stays with the institution.

\vspace{6pt}
\noindent\textbf{Keywords:} text-to-SQL; large language models; clinical data warehouse;
hallucination; metadata; statistical criteria; hospital information systems;
retrieval-augmented generation; hospital management; performance measurement
...
team of data engineers; over the 2023--2024 period,
\PH{XX: share waiting over five working days}\% of requests waited more than five
working days for a first result, and \PH{XX: share needing a further clarification
round}\% required at least one round of criterion clarification after delivery. These delays defer
...
Delay is not the whole cost. Hospital management runs on indicators that are
compared: one department against another in performance review, one month against
the last in quality control, and one institution against its peers in regulatory
returns. A comparison carries information only if the counting criterion behind
each number is the same, and when every report is written by hand that criterion
lives in the query rather than in any catalog the institution can inspect. Two
analysts asked in different quarters for the volume of emergency surgery may
filter on different flags and each be able to defend the choice; what management
then reads as a trend is an artifact of who wrote the query. The failure is quiet,
because a query built on the wrong criterion still runs and still returns a
plausible number. What a reporting pipeline owes a hospital is therefore not only
the number but the criterion it was counted by, in a form the owner of the
indicator can review---and, where the criterion is genuinely contested, a way of
saying so instead of choosing one silently.
...
and has been carried into the clinical setting through electronic health record
(EHR)-specific datasets,
generation models trained on them, and reliability-oriented shared tasks that
reward abstention on unanswerable questions
\cite{treqs2020,medts2021,ehrsql2024,ragepi2024,rag2step2025}. The evaluation
framework used here is adopted from one of those studies, which converted
clinical-trial eligibility criteria into Observational Medical Outcomes Partnership
Common Data Model queries and profiled hallucination across eight models under
five prompting strategies \cite{lee2025}: the eight-model cloud-versus-local
comparison, the five prompting strategies, the split into development and
validation task sets, and the five-level severity ordering of hallucination
categories are theirs, and we reuse them so that our numbers can be read beside
theirs. What differs is the object being queried. Their criteria are eligibility
rules expressed against a shared common data model, where a concept either exists
in the vocabulary or does not; ours are institution-specific counting criteria,
where the field exists, the query executes, and the number is still wrong. That
difference is what Category B of our taxonomy measures, and it has no counterpart
in a shared-vocabulary setting. A separate evaluation of six recent models on
MIMIC-III and TREQS reports the same accuracy--cost tension and names
retrieval augmentation and terminology integration as the remedies worth trying
\cite{tankovic2025}; the system reported here implements both, and quantifies
what each contributes. A parallel line
...
validation\_focused, and error\_aware). The factorial layout, the split between
cloud-hosted and locally deployed models, and the five strategies and their names
are taken unchanged from \cite{lee2025} so that the two sets of results are
directly comparable; the models, the tasks, and the warehouse are ours. The five
prompting strategies were designed to test different approaches to query
generation:
...
large-scale model comparison, we classified hallucinations into five categories
ordered by severity, following the schema of \cite{lee2025} and substituting our
own Category B: theirs records a valid concept assigned to the wrong domain, which
is a vocabulary error, whereas ours records a valid field assigned to the wrong
counting criterion, which is a definition error and the failure mode this study is
about. Category A (Critical) comprised
...
query generation. Information extraction from these 66 preprocessed criteria
...
These factors can only reduce real-world performance relative to the synthetic
validation, and the present study does not establish by how much: bounding the
gap needs a prospective run against production tables with adjudicated ground
truth, which is the next step we plan. SynHDW's
...
\subsection{Implications for Hospital Management}\label{sec:mgmt}
The measurements above were taken to characterise a generation pipeline, but the
decisions they bear on are managerial, and the translation is not automatic. Four
of them are worth stating in the terms an information-management office would use.

First, the abstention path is an escalation route, not a safety guarantee. Of 791
generated queries, 132 (16.7\%) raised a TO-CONFIRM item and 104 of those
(78.8\%) were warranted, which means that in most cases where the pipeline
declared a criterion contested, an indicator owner was in fact the right person to
decide. The complementary figure is the one that governs how the output must be
handled: recall was 63.0\%, so 61 genuinely ambiguous requests were resolved
without asking, and in 42 of those the criterion chosen was wrong. Abstention
therefore reduces the volume of silent criterion decisions rather than eliminating
it, and a reporting workflow built on this pipeline still needs certification
before an indicator leaves the institution.

Second, the ablation identifies which governance asset earns the accuracy. Adding
the schema to the prompt raised the effective SQL rate from 22.5\% to 40.8\% and
nearly eliminated nonexistent-field errors, but it did not touch wrong criteria;
those halved only when metadata retrieval and historical SQL were added, at
60.8\%, and verification carried the rate to 71.7\%. The artifact that buys
criterion correctness is thus the curated metadata knowledge base and the archive
of certified queries---both of which are maintained by people, on a budget, as an
ongoing institutional commitment rather than a one-off build. The ablation gives
that commitment a measured return, which is the form in which it can be argued for.

Third, the cost figures relocate the bottleneck rather than removing it. Manual
authoring takes about 1.8 hours of engineer time per task, and at 650 requests per
month the queue is bounded by engineer-hours; a first draft in under two minutes
removes that bound. What it does not remove is the need for someone with authority
over the indicator to adjudicate the criterion, and Table~\ref{tab:mgmt} shows
every management use of the output resting on exactly that step. Raising
throughput without also raising adjudication capacity converts a visible backlog
into an invisible error rate.

Fourth, one of our failures was a data-governance finding rather than a model
finding. The unplanned-reoperation query retrieved nothing (Jaccard 0.00) because
the model selected a planning flag that is structurally valid, passes schema
validation, and is unpopulated across the warehouse. No amount of prompting
repairs that; what it identifies is a field the institution believes it collects
and does not. A pipeline that grounds generation in metadata will keep surfacing
such gaps, and treating them as defects in the data-governance process is more
useful than treating them as defects in the generator.

Table~\ref{tab:mgmt} sets out the management uses the present results support,
each against the reliability actually measured for it and the residual risk that
stays with the manager. Every figure in the table is taken from
Tables~\ref{tab:domains}--\ref{tab:ablation}; the table adds no new measurement.

\begin{table}[!ht]
\caption{Management uses supported by the present results. Each row pairs a
decision the output feeds with the reliability measured for it in this study and
the risk that is not transferred to the system. No figure in this table is new;
all are drawn from the Results.}
\label{tab:mgmt}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{2.4cm}L{3.3cm}L{3.3cm}L{3.3cm}}
\toprule
Management use & What the pipeline supplies & Reliability measured here &
Residual risk retained by the manager\\
\midrule
Departmental performance review &
Indicator query with the source field recorded for every criterion &
Term mapping 52.3\% overall, 70.7\% in surgery and anesthesia, 34.5\% in
laboratory &
Wrong criteria are 34.7\% of all errors and remain executable, so a reported
number can be wrong without the query failing\\
\addlinespace
Quality-control monitoring over time &
Reusable query text, so the same criterion is applied in successive periods &
Effective SQL rate 71.7\% (95\% CI 63.0--79.0) for the on-premises model &
Each cell was run once under deterministic decoding, so run-to-run variability is
unestimated\\
\addlinespace
Regulatory and external reporting &
Explicit field provenance for audit of how an indicator was computed &
Agreement with engineer-certified SQL was high for single-criterion indicators;
Jaccard 0.00 and 0.08 for two compound ones &
Compound institution-specific definitions still require engineer certification
before external release\\
\addlinespace
Criterion governance and catalog upkeep &
TO-CONFIRM referrals routed to the indicator owner and recorded &
132 of 791 raised (16.7\%), 78.8\% warranted, recall 63.0\% &
61 ambiguous requests were not referred, and 42 of those took the wrong
criterion\\
\addlinespace
Analyst capacity planning &
First draft in under 2 min against about 1.8 h of engineer authoring &
About US~\$117 per month at 650 requests for gpt-4o, about US~\$8 for
deepseek-v3, none for the on-premises model &
Savings are realised only if criterion adjudication capacity grows with
throughput\\
\bottomrule
\end{tabular}
\end{table}
...
indicators. For hospital management the operative conclusion is narrower than the
headline rate: the pipeline moves the constraint on reporting from engineer-hours
to criterion adjudication, and the measured abstention recall of 63.0\% means that
authority over indicator definitions has to be exercised, not delegated to the
generator. Section~\ref{sec:mgmt} states which management uses the present
evidence supports and which risk stays with the institution.
...
S.L. is the sole author and carried out, in CRediT terms, conceptualization,
methodology, software, validation, formal analysis, investigation, data
curation, visualization, and writing---both original draft preparation and
review and editing. The author has read and agreed to the published version of
the manuscript.
...
approved by the Institutional Review Board of West China Hospital, Sichuan University (protocol code
\PH{XXXX-XXXX}, approved 27 July 2026).
...
the SynHDW generator and its schema manifest, is available from the
corresponding author upon reasonable request, subject to the data-governance
rules of West China Hospital, Sichuan University. The de-identified metadata schema, the
metric-criterion dictionary and the evaluation task set are available under the
same conditions. Patient-level warehouse data cannot be shared.

\section*{Acknowledgments}
The author thanks colleagues in the Department of Information and Data
Management, West China Hospital, Sichuan University, for access to the reporting
ticket archive and for discussions on statistical criteria.

\section*{Conflicts of Interest}
A related invention by the author (patent AJ2534335, a large language model--
...
learning. The author declares no other conflict of interest. The funders had no
...
language editing and for drafting sections of the text, which the author
subsequently reviewed and revised. No text, data, figure or citation was
accepted without author verification, and the author takes full responsibility
