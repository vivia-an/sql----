...
----------------------------------------------------------------
  -- 查询逻辑说明：
  -- 指标名称：患者入院8小时内查房率（按月统计）
  -- 计算方法：统计每个月，入院8小时内至少开具1条“检查”或“治疗”相关医嘱的患者人次数（分子），
  --          与同期入院且已出院的患者总人次数（分母）之比，再乘100形成百分比。
  --
  -- 详细逻辑与血缘关系：
  -- 1. 分母数据来源（base_inpatients）：
  --    - 来源表：datacenter_db.Visit_IPReg
  --    - 过滤：已出院（Visit_IPReg_OutHospDtTm非空）、逻辑删除筛选（Visit_IPReg_IsDeleted='0'）、入院时间有效
  --    - 字段：“统计月份”取自入院时间，"Visit_IPReg_VisitID"作为唯一标识，带出"入院时间"、"出院时间"、"入院科室名称"
  --
  -- 2. 分子候选医嘱数据（order_records）：
  --    - 来源表：datacenter_db.Order_Main
  --    - 过滤：未逻辑删除、存在医嘱开立时间（Order_Main_OrderDtTm非空）
  --    - 类型判定：医嘱大类名称/医嘱类型名称包含“检查”或“治疗”，或申请单类型为常见“查房相关”申请单
  --    - 字段：关联VisitID、开医嘱时间、医嘱分类和申请类型
  --
  -- 3. 8小时内医嘱患者筛选（orders_within_8h）：
  --    - JOIN：base_inpatients与order_records，按VisitID匹配
  --    - 时间判定：开医嘱时间-入院时间区间为0-8小时（DATE_DIFF计算）
  --    - 保证每人只计一次（DISTINCT）
  --
  -- 4. 按月汇总（monthly_stats）：
  --    - 每月分母=同期出院患者总人次数（COUNT DISTINCT VisitID）
  --    - 每月分子=同期入院8小时内开具医嘱人次数（COUNT DISTINCT VisitID满足8小时条件）
  --
  -- 5. 最终结果输出：
  --    - 显示：统计月份、分子人数、分母人数、百分比（查房率），按月份倒序
  --
  -- 关键信息与表血缘归属：
  --    - 入院/出院/科室/VisitID来源：datacenter_db.Visit_IPReg
  --    - 医嘱开立时间、类型、申请单来源：datacenter_db.Order_Main
  --    - 所有表与字段均为大数据集成、varchar类型，均做了必要的逻辑删除过滤
  --    - 中文字段名均双引号包裹，兼容Presto
  --    规则：给出大类类别 补全规则
  ----------------------------------------------------------------
